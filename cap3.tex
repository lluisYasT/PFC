...
En este sketch utilizamos las funciones de Arduino \comando{pinMode},\comando{digitalWrite} y \comando{delay}. En la función \comando{setup()} inicializamos el puerto al que está conectado el LED como una salida\protect\footnote{PIN\_LED1 está definido en el archivo de cabecera Board\_Defs.h que depende del modelo de microcontrolador que estemos usando. Generalmente este es el LED que se encuentra en la propia placa, conectado al pin 13.}, mientras que en \comando{loop()} encendemos y apagamos el LED con retardos de un segundo.
...
Para poder observar las acciones que lleva acabo \programa{MPIDE} durante la compilación del programa podemos mantaner apretada la tecla \comando{Mayus} mientras pulsamos el botón de compilar. De esta forma \programa{MPIDE} nos mostrará todos los comandos que ejecute, en el terminal.
...
\begin{itemize}
    ...
    \item \negrita{-mno-smart-io}. Esta opción deshabilita el análisis estadístico de las cadenas con formato que son pasadas a \comando{printf} o \comando{scanf} y a las variaciones `f' y `v' de estas funciones.
    ...
    \item Parámetros de enlazado:
        \begin{itemize}
            ...
            \item \negrita{-T}. Con esta opción podemos especificar un script de enlazado. Se usa dos veces: Una para el script \comando{chipKIT-application-COMMON.ld} y otra para\\ \comando{chipKIT-application-32MX795F512.ld}.
        \end{itemize}
\end{itemize}
...
El resultado de ejecutar estos dos comandos es el archivo \comando{sketch.hex} que ya podrá ser cargado al microcontrolador.
...
Para poder realizar todo el proceso con \programa{make} se necesita un archivo \comando{Makefile} en el que se describe como llevarlo acabo. Este archivo se escribe de la siguiente forma:
\begin{listing}[language=make]
objetivo: dependencia_1 dependencia_2 ...
	comandos
\end{listing}
Esta estructura la podemos repetir para todos los objetivos que sean necesarios. El objetivo es lo que se quiere conseguir a través de los comandos, pero para poder ejecutarse es necesario que se cumplan las dependencias (que pueden ser archivos u otros objetivos). En el apéndice \ref{chap:apendice2} se encuentra el Makefile completo y su funcionamiento.

\programa{make}, siguiendo las instrucciones que se encuentran en el \comando{Makefile} se encarga de compilar, enlazar y empaquetar la biblioteca core en el archivo \comando{core.a} y luego compila todos los archivos en la carpeta \comando{lib} y la carpeta{src}. Es decir, realiza las mismas operaciones que \programa{MPIDE} a excepción de la primera de ellas (Buscar funciones en el sketch).

En la plantilla también he incluido un script escrito en Perl que puede ser ejecutado de forma explícita por el usuario y que permite replicar este comportamiento. El script se puede ejecutar sobre el archivo \comando{src/sketch.pde} si existe y genera el fichero en código fuente \comando{src/sketch.cpp} siempre y cuando este último archivo, en caso de existir, esté vacío.

Con el \comando{Makefile} incluido podemos ejecutar diversas acciones dependiendo del objetivo que elijamos. Si se ejecuta \comando{make src/sketch.S} (se puede hacer con cualquier archivo escrito en C o C++ que se encuentre en la carpeta) se compila pero no ensambla ni enlaza el archivo \comando{sketch.cpp}. Del mismo modo, si llamamos a la orden \comando{make src/sketch.o} se compila y ensambla \comando{sketch.cpp}. \comando{make link} compilará, ensamblará y enlazará el programa guardando el resultado en \comando{bin/main.elf}. Este último es el objetivo por defecto, por lo que si ejecutamos \comando{make} sin ningún argumento, el resultado será el mismo.

Una vez enlazado podemos usar la orden \comando{make load} para cargar el programa en la placa conectada al puerto USB.\footnote{En el \comando{Makefile} ya hay dos puertos serie predefinidos según la plataforma. Solo hace falta modificar una variable para cambiar el puerto al que está conectada la placa.} Con esta orden se ejecuta \comando{avrdude} con los mismos parámetros que si fuese llamado a través de \programa{MPIDE}.
