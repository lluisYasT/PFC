...
\section{Software}
A lo largo del proyecto utilizaremos dos entornos de desarrollo integrado: MPIDE y MPLAB X.
\subsection{MPIDE}
MPIDE\cite{website:mpide} es el IDE creado por Digilent para el entorno ChipKIT. Su diseño es idéntico al IDE de Arduino, variando solamente el color y el compilador usado.
\figura{mpide.jpg}{scale=0.6}{MPIDE}{mpide}{H}
MPIDE es un entorno de desarrollo sencillo y con unas características muy básicas y limitadas. Con él podremos escribir nuestro programa, compilarlo y cargarlo en nuestro microcontrolador sin necesidad de ninguna otra aplicación, siempre y cuando hayamos seleccionado el modelo adecuado y el puerto serie en las opciones del programa. \\
Al igual que el entorno de Arduino, no es posible depurar programas, para eso usaremos MPLAB X. También dispone de un monitor para el puerto serie, con el que podemos leer lo que el microcontrolador envía a través de este puerto y enviar información al microcontrolador.
\subsection{MPLAB X}
MPLAB X\cite{website:mplab} es la última versión del entorno de desarrollo integrado de Microchip. A diferencia de MPIDE, este programa es un IDE completo, basado en Netbeans. Está pensado para ser utilizado con toda su gama de microcontroladores y, con la ayuda de un depurador/programador como el chipKIT PGM podremos depurar nuestros programas. Si queremos, también podemos usar los productos chipKIT como cualquier otro microcontrolador de Microchip, escribiendo los programas en este IDE y compilándolos con el compilador que tiene Microchip disponible en su página web, C32\cite{website:c32}.
\figura{mplab.jpg}{scale=0.35}{MPLAB X Depurando}{mplab}{H}

Como se aprecia en la Figura \ref{mplab} MPLAB X nos ofrece mucha más información y posibilidades que MPIDE. En esta figura vemos a MPLAB X en mitad de la depuración del programa Blink, que fue compilado usando MPIDE.\\
...
\subsection{Core}
Para empezar, tenemos los archivos que formarán parte de la biblioteca ``Core'' a la hora de la compilación. Estos archivos vienen incluidos con MPIDE y los podemos encontrar en el directorio en el que tengamos instalado MPIDE, en la subcarpeta:\\
...
\begin{tabular}{| c || c |}
    ...
    \hline
    Windows & C:\textbackslash Archivos de Programa\textbackslash MPIDE\textbackslash hardware\textbackslash pic32\textbackslash cores\textbackslash pic32\\
    \hline
  \end{tabular}
\end{center}
...
\subsection{Sketch}
Los programas que escribimos usando MPIDE (o el IDE de Arduino) se llaman sketches. En realidad, no escribimos un programa completo porque, como acabamos de ver el programa en sí ya está escrito en la biblioteca Core. Nuestro objetivo es implementar dos funciones: setup() y loop(), que ya están declaradas en el archivo de cabecera wiring.h.\\
...
En setup() colocaremos el código para la inicialización de nuestro programa. Es el lugar donde debemos configurar el puerto serie, por ejemplo o definir como se comportaran los pines (entrada o salida), entre otras cosas.\\
En loop() pondremos el código que se ejecutara de forma cíclica indefinidamente ya que, como podemos observar en main.cpp, la función loop() se encuentra dentro de un bucle infinito.\\

Veamos como ejemplo el sketch Blink:
\lstinputlisting[language=C++, caption=Blink.pde]{codigo_fuente/Blink.pde}

En este sketch utilizamos las funciones de Arduino pinMode,digitalWrite y delay. En la función setup inicializamos el puerto al que está conectado el LED como una salida\footnote{PIN\_LED1 está definido en el archivo de cabecera Board\_Defs.h que depende del modelo de microcontrolador que estemos usando}, mientras que en loop encendemos y apagamos el LED con retardos de un segundo.

\subsection{Bibliotecas de terceros}

\section{Proceso de compilación de MPIDE}
En este apartado trataremos de describir el proceso que sigue MPIDE para la compilación y posterior carga en el microcontrolador del programa creado en MPIDE.  
